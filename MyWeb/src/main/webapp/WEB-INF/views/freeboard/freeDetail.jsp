...
<script>
	...
	$(document).ready(function() {
		$('#replyRegist').click(function() {
	        ...
	        const id = $('#replyId').val(); //작성자
			const pw = $('#replyPw').val(); //비밀번호
			...
			*/
			$.ajax ({
				...
				data: JSON.stringify( //이렇게 직접 객체를 만드는 것도 가능하다. 미리 만들어서 전달하는 거랑 같다.
					{
						"bno": bno,
						"reply": content,
						"replyId": id,
				...
				contentType: 'application/json', //전송 타입
		...
		//더보기 버튼 클릭 처리(클릭 시 전역 변수 page에 +1 한 값을 전달)
		$('#moreList').click(function() {
			//왜 false를 주었는가?
			//더보기이기 때문에 누적해야 하기 때문에
			//1페이지의 댓글 내용 밑에다가 2페이지를 줘야한다. 1페이지를 없애고 2페이지를 보여주자 는게 아니다.
			getList(++page, false);
		});
		...
		function getList(pageNum, reset) { //댓글 목록을 불러오는 메소드
			...
			$.getJSON(
				...
				function(data) { //서버가 전송한 데이터가 data에게 반환.
					...
					//페이지 번호 * 데이터 수보다 전체 댓글 개수가 작으면 더보기 버튼을 없애자.
					...
					//응답 데이터의 길이가 0같거나 더 작으면 함수를 종료하자.
		...
		*/
		...
		$('#replyList').on('click', 'a', function(e) {
			...
			//2.모달 창 하나를 이용해서 상황에 따라 수정, 삭제 모달을 구분하기 위해 조건문을 작성. (모달 하나로 수정, 삭제 같이 처리. 그러기 위해 디자인 조정)
			//hasClass()는 클래스 이름에 매가값이 포함되어 있다면 true, 없다면 false.
			if($(this).hasClass('replyModify')) {
				...
				//BootStrap에서는 trigger를 통해서 모달을 열고 닫았지만, 지금은 그런게 없기 때문에
				//제이쿼리를 이용하여 직접 모달을 열고 닫습니다.
				$('#replyModal').modal('show'); //모달을 열어주는 함수.
				
			} else {
				//삭제버튼을 눌렀으므로 삭제 모달 형식으로 변경
		...
		//수정 처리 함수(수정 모달을 열어서 수정 내용을 작성 후 수정버튼을 클릭했을 때)
		...
		function timeStamp(millis) {
			...
			if(gap < 60 * 60 * 24 * 1000) { //1일 미만일 경우
				...
				const today = new Date(milies);
				const year = today.getFullYear(); //년
				const month = toady.getMoun //월
