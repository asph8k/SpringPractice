...
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>  
   ...
   <script>
      $(function() {
         ...
         //사용자가 페이지 관련 버튼을 클릭했을 때, 기존에는 각각의 a태크의 href에다가
         //각각 다른 url을 작성해서 요청을 보내줬다면, 이번에는 클릭한 그 버튼이 무엇인지를 확인해서
         //그 버튼에 맞는 페이지 정보를 자바스크립트로 끌고와서 요청을 보내 준다.
         $('#pagination').on('click', 'a', function(e) {
			e.preventDefault(); //a태그의 고유기능 중지.
			//현재 이벤트가 발생한 요소(버튼)의 data-pageNum의 값을 얻어서 변수에 저장
			//const value = e.target.dataset.pageNum; //바닐라 방식 -> Vanilla JS
			const value = $(this).data('pagenum'); //제이쿼리 방식 -> jQuery
			
			//페이지 버튼들을 감싸고 있는 form태그를 name으로 지목하여
			//그 안에 숨겨져있는 pageNum이라는 input태그의 value에 위에서 얻은 data-pageNum의 값을 삽입 한 후 submit
