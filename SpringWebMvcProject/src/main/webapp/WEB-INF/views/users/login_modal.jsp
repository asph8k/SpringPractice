...
<script>
	...
	$(function() {
		...
		//각 입력값들의 유효성 검증을 위한 정규표현식을 변수로 선언. //정규 표현식(Regular Expression) 줄여서 RegExp
		// /^가 시작 $/ 끝을 의미. []안에 작성된 내용은 허용된 문자 {} 글자의 길이
		...
		$('#user_id').keyup(function() {
			...
			//정규표현식변수.test('검증하고 싶은 값') -> return boolean type으로 리턴
			...
			//ID 중복 확인 (비동기 처리)
			//비동기 : 특정 로직의 실행이 끝날 때 까지 기다리지 않고 먼저 코드를 실행.(페이지 전환 없이 통신)
			else {
				//ID 중복 확인 통신을 위해 사용자의 입력값을 먼저 가져오기
				const id = $(this).val();
				
				//ajax 호출.
				//클라이언트에서 서버와 비동기 통신을 진행하게 도와주는 ajax함수
				$.ajax({
					...
					data: id, //서버로 전송할 데이터. 위에 선언한 변수명 id
					success: function(result) { //매개변수에 통신 성공 시 리턴되는 데이터가 저장됨.
						//서버와 통신 성공 시 실행할 내용.
					...
					error: function(status, error) { //통신에 실패시 실행 할 로직
						console.log('통신 실패!');
						console.log(status, error);
					}
				}); //end ajax (아이디 중복 확인 비동기)
			}
		}); //아이디 검증 끝. keyup function
		...
		//사용자가 입력하는 4가지 데이터중 단 하나라도 문제가 있다면 회원가입 처리를 해주지 말아야 한다.
		$('#signup-btn').click(function() {
			if(chk1 && chk2 && chk3 && chk4) {
				...
				$.ajax({
					...
					dataType: 'text',
					//JavaScript 객체를 JSON 문자열로 변환해 주는 메소드
					data: JSON.stringify(user),
					success: function(result) {
						console.log('통신 성공!: ' + result);
						alert('회원가입을 환영합니다.');
						location.href='/'; // '/' = home.jsp을 보여준다.
					},
					error: function() {
						alert('회원가입 실패');
					}
